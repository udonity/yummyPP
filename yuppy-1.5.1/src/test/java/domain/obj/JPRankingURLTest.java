...
class JPRankingURLTest {
	
	JPRankingURL sut;
	
	@BeforeEach
	void setup() {
		sut = new JPRankingURL(1);
	}
	...
	void コンストラクタでrankが1の時URLが整形される() throws MalformedURLException {
		...
		URL expected = new URL("https://www.scoresaber.com/global/1&country=jp");
		// verify
	...
	void pageが1の時setURLで51をセットすると2ページ目になる() throws MalformedURLException {
		...
		sut.setURL(51);
		URL actual = sut.getURL();
		URL expected = new URL("https://www.scoresaber.com/global/2&country=jp");
		// verify
		assertThat(actual, is(expected));
	}
